package com.viewol.shiro.cache;

import com.viewol.shiro.utils.LoggerUtils;
import com.viewol.shiro.utils.SerializeUtil;
...
//            jedis.select(dbIndex);
    ...
    public <T> T get(int dbIndex, String key , Class<T>...requiredType){
        Jedis jds = null;
        boolean isBroken = false;
        try {
            jds = getJedis();
//            jds.select(dbIndex);
     ...
     * @param timer （秒）
     */
    public void setex(int dbIndex, Object key, Object value, int timer) {
        Jedis jds = null;
        boolean isBroken = false;
        try {
            jds = getJedis();
