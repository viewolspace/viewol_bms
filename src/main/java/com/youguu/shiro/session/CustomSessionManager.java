package com.youguu.shiro.session;


import com.youguu.shiro.utils.LoggerUtils;
import com.youguu.shiro.utils.StringUtils;
...
public class CustomSessionManager {
	 ...
	 * session status
	 */
	public static final String SESSION_STATUS = "sun_ms_online_status";
	 ...
	 * @return
	 */
	public List<OnlineSysUser> getAllUser() {
		//获取所有session
		Collection<Session> sessions = customShiroSessionDAO.getActiveSessions();
		List<OnlineSysUser> list = new ArrayList<>();

		for (Session session : sessions) {
			OnlineSysUser bo = getSessionBo(session);
			if (null != bo) {
				list.add(bo);
			}
		}
	 ...
	 * @return
	 */
	public OnlineSysUser getSession(String sessionId) {
		Session session = shiroSessionRepository.getSession(sessionId);
		OnlineSysUser bo = getSessionBo(session);
		return bo;
	}

	private OnlineSysUser getSessionBo(Session session) {
		//获取session登录信息。
		Object obj = session.getAttribute(DefaultSubjectContext.PRINCIPALS_SESSION_KEY);
		if (null == obj) {
			return null;
		...
		if (obj instanceof SimplePrincipalCollection) {
			...
			if (null != obj && obj instanceof SysUser) {
				//存储session + user 综合信息
				OnlineSysUser userBo = new OnlineSysUser((SysUser) obj);
				//最后一次和系统交互的时间
	...
	public void forbidUserById(Integer id, Long status) {
		//获取所有在线用户
		for (OnlineSysUser bo : getAllUser()) {
